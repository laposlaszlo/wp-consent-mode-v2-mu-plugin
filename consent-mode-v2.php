...
function cmv2_get_options()
{
    ...
    $saved = get_option('cmv2_settings', []);
...
function cmv2_settings_page()
{
    ...
    // Beállítások mentése
    if (isset($_POST['cmv2_save_settings']) && check_admin_referer('cmv2_settings_action', 'cmv2_settings_nonce')) {
        $options = cmv2_get_default_options();
        $saved = [];

        foreach ($options as $key => $default) {
            if (isset($_POST['cmv2_' . $key])) {
                $value = $_POST['cmv2_' . $key];

                // Sanitizálás típus szerint
        ...
        update_option('cmv2_settings', $saved);
        echo '<div class="notice notice-success is-dismissible"><p>Beállítások sikeresen mentve!</p></div>';
    }

    // Beállítások exportálása
    if (isset($_POST['cmv2_export_settings']) && check_admin_referer('cmv2_settings_action', 'cmv2_settings_nonce')) {
        $settings = get_option('cmv2_settings', []);
    ...
    if (isset($_POST['cmv2_import_settings']) && check_admin_referer('cmv2_settings_action', 'cmv2_settings_nonce')) {
        if (isset($_FILES['cmv2_import_file']) && $_FILES['cmv2_import_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($imported && is_array($imported)) {
                update_option('cmv2_settings', $imported);
                echo '<div class="notice notice-success is-dismissible"><p>Beállítások sikeresen importálva!</p></div>';
            } else {
                echo '<div class="notice notice-error is-dismissible"><p>Hibás fájlformátum!</p></div>';
            }
        }
    }
    ...
    <div class="wrap">
        ...
        <form method="post" action="" enctype="multipart/form-data">
            <?php wp_nonce_field('cmv2_settings_action', 'cmv2_settings_nonce'); ?>
...
add_action('admin_enqueue_scripts', function ($hook) {
    if ($hook !== 'settings_page_cmv2-settings') {
        return;
    }
...
add_filter('plugin_action_links_' . plugin_basename(__FILE__), function ($links) {
    $settings_link = '<a href="' . admin_url('options-general.php?page=cmv2-settings') . '">Beállítások</a>';
...
add_action('admin_notices', function () {
    $opts = get_option('cmv2_settings');
    if (empty($opts) && current_user_can('manage_options')) {
        $screen = get_current_screen();
        if ($screen && $screen->id !== 'settings_page_cmv2-settings') {
    ?>
            <div class="notice notice-info is-dismissible">
                <p>
                    <strong>Consent Mode V2:</strong>
                    A plugin aktív! <a href="<?php echo admin_url('options-general.php?page=cmv2-settings'); ?>">Kattints ide</a>
